...
def data_generation(rgbs,masks,labels,countour_points,roi_dimensions,num_objs,num_data,img_counter):    
    ...
    for data_id in range(num_data):
        ...
        for i in range(num_objs):
            ...
            ## in case roi is larger than background size
            roi_height = min(roi_height, crop_height)
            roi_width = min(roi_width, crop_width)

            # ratio_x = random.random()
            # ratio_y = random.random()

            ### avoid self-occlusion
            # max_trails = 20 ## try maximum 10 times until no self-occlusion happens
            max_trails = 200 ## try maximum 10 times until no self-occlusion happens
            ...
            ## in case roi is larger than background size
            random_background[translate_roi_min_y:translate_roi_max_y,translate_roi_min_x:translate_roi_max_x] = rgb_roi[:roi_height, :roi_width]

            # random_background_mask[translate_roi_min_y:translate_roi_max_y,translate_roi_min_x:translate_roi_max_x] = mask_roi
            ## in case roi is larger than background size
            random_background_mask[translate_roi_min_y:translate_roi_max_y,translate_roi_min_x:translate_roi_max_x] = mask_roi[:roi_height, :roi_width]
...
if __name__ == "__main__":
    ...
    for obj_num in glob.glob(os.path.join(rgb_data_path, '*')):
        for rgbfile in glob.glob(os.path.join(obj_num, '*jpg')):
            ...
            rgbRmBgfile = rgbfile.replace("rgb","rgb_rm_bg")
            rgbRmBgfile = rgbRmBgfile.replace("jpg","png")
            rgb_img_rm_bg = cv2.imread(rgbRmBgfile, cv2.IMREAD_UNCHANGED)
            # trans_mask = rgb_img_rm_bg[:,:,3] ==0
            trans_mask = rgb_img_rm_bg[:,:,3] < 100
            rgb_img_rm_bg_thre = np.zeros((rgb_img_rm_bg.shape[0], rgb_img_rm_bg.shape[1],3), np.uint8)
            rgb_img_rm_bg_thre[:] = [255, 255, 255]
            rgb_img_rm_bg_thre[trans_mask] = [0, 0, 0]
            kernal = np.ones((9, 9), np.uint8)
            rgb_img_rm_bg_thre = cv2.morphologyEx(rgb_img_rm_bg_thre, cv2.MORPH_CLOSE, kernal)

            # rgb_img_rm_bg_rgb = cv2.imread(rgbRmBgfile)
            rgb_img_rm_bg_rgb = rgb_img_rm_bg[:,:, :3]
            rgb_img_rm_bg_rgb[trans_mask] = [255, 255, 255]
            ## save extacted object with white background
            rgb_img_rm_bg_rgb_file = rgbfile.replace("rgb","rgb_rm_bg_white")
            file_length = len(rgb_img_rm_bg_rgb_file.split("/")[-1])
            isExist = os.path.exists(rgb_img_rm_bg_rgb_file[:-file_length])
            if not isExist:
                # Create a new directory because it does not exist 
                os.makedirs(rgb_img_rm_bg_rgb_file[:-file_length])
            cv2.imwrite(rgb_img_rm_bg_rgb_file, rgb_img_rm_bg_rgb)

            ## remove black edge generated by rembg operation
            # kernal = np.ones((1, 1), np.uint8)
            kernal = np.ones((2, 2), np.uint8)
            rgb_img_rm_bg_rgb = cv2.morphologyEx(rgb_img_rm_bg_rgb, cv2.MORPH_CLOSE, kernal)

            gray = cv2.cvtColor(rgb_img_rm_bg_thre, cv2.COLOR_BGR2GRAY)
            ...
            min_x = sorted_point[0][0]
            min_y = sorted_point[0][1]
            max_x = sorted_point[-1][0]
            max_y = sorted_point[-1][1]
            if(min_x<0):
                min_x = 0
            if(min_y<0):
                min_y = 0
            ...
            min_extract_x = sorted_point[0][0] - (sorted_point[-1][0] - sorted_point[0][0])
            min_extract_y = sorted_point[0][1] - (sorted_point[-1][1] - sorted_point[0][1])
            max_extract_x = sorted_point[-1][0] + (sorted_point[-1][0] - sorted_point[0][0])
            max_extract_y = sorted_point[-1][1] + 10
            min_object_x = sorted_point[0][0] - max(min_extract_x, 0)
            min_object_y = sorted_point[0][1] - max(min_extract_y, 0)
            max_object_x = min(min_object_x + width, rgb_img_rm_bg.shape[1])
            max_object_y = min(min_object_y + height, rgb_img_rm_bg.shape[0])

            roi_dimension  = [min_y,min_x,height,width]
            # target_obj = rgb_img[min_y:max_y,min_x:max_x]
            print (rgbRmBgfile)
            target_obj = rgb_img_rm_bg_rgb[min_y:max_y,min_x:max_x]
            target_obj_mask = mask_img[min_y:max_y,min_x:max_x]
            
            target_obj_rgbs.append(target_obj)
            target_obj_rgbs_rm_bg.append(target_obj)
